<script>
  ...
  function checkPin() {
    if (pin.length == 6) {
      axios.post("/auth/pin", { pin: pin }).then(
        (response) => {
        ...
        (error) => {
          ...
          {
            errors = "Sesi anda telah berakhir";
          }
  ...
  function deleteSession() {
    ...
    axios.delete("/auth/session/" + session_id);
  }
</script>
    ...
    >
      ...
      <div class="py-6 w-full max-w-md relative">
        ...
        <div class="mb-8 text-center">
          ...
          >
            <span>{user.Pin ? "Masukan PIN" : "Buat PIN Baru"}</span>
          </h1>
        </div>
        <!-- END Header -->
        ...
        <div class="flex flex-col rounded shadow-sm bg-white overflow-hidden">
          <div class="p-6 grow w-full">
            ...
            {#if errors}
              <div
                class="p-4 md:p-5 mb-6 rounded text-orange-700 bg-orange-100"
              >
            ...
            <div class="flex justify-center">
              <input
                type="text"
                class="w-1/2 text-3xl mx-auto border px-3 py-2"
                bind:value={pin}
                on:input={checkPin}
              />
            </div>
            <div class="grid grid-cols-3 mt-6">
              {#each [1, 2, 3, 4, 5, 6, 7, 8, 9, "*", 0, "#"] as numbr}
                <button
                  ...
                  class="border cursor-pointer -mx-px flex items-center justify-center py-3 text-3xl"
                  >{numbr}</button
                >
              {/each}
            </div>
