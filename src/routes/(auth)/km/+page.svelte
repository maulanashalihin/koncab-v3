<script>
    ...
    let editkmModal = false;
 
    function savekm() {
       active_km.peserta = active_km.list_peserta.length;
       active_km.bulan = dayjs(active_km.tanggal).format("MMMM YYYY");
       active_km.id =
          dayjs(active_km.tanggal).format("YYYY-MM-DD-") +
 ...
 <Modal width="max-w-lg" bind:show={editkmModal} title="Edit km">
    <form on:submit|preventDefault={savekm} class="p-4 space-y-4">
       <div class="space-y-1">
          <label for="tanggal" class="font-medium">Tanggal</label>
          <input
             required
             bind:value={active_km.tanggal}
       ...
       <div class="space-y-1">
          ...
          <input
             required
             bind:value={active_km.pembicara}
             class="bg-gray-50 border border-gray-300 outline-none text-gray-900 text-sm rounded-lg focus:ring-cyan-500 focus:border-cyan-500 block w-full p-2.5 placeholder-gray-400"
             type="text"
             id="pembicara"
