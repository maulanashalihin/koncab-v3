   ...
   function savekontak() {
      if (active_kontak.id) {
         ...
         if(is_mundur)
         {
            ...
            db.im.delete(active_kontak.id);

            Log("im", active_kontak, "delete");

            editkontakModal = false;
            return;
         }

         db.im.put(active_kontak);
      } else {
         active_kontak.id = active_kontak.name;
         active_kontak.createdAt = dayjs().format("YYYY-MM-DD");
         db.im.add(active_kontak);
      }
      Log("im", active_kontak);
      editkontakModal = false;
      Loadkontak();
   }
      ...
      <div class="mt-10 overflow-x-auto">
         ...
         {:else}
            <div class="border border-dashed py-20 text-gray-500">
               <div class="text-center">Tidak ada im</div>
            </div>
         {/if}
      </div>
   </div>
...
<Modal width="max-w-lg" bind:show={editkontakModal} title="Edit im">
